    ...
    <section style={{ backgroundColor: 'transparent' }} className="relative mt-24">
      ...
      <section className="pt-28 pb-28 relative">
        <div className="container mx-auto px-8 md:px-32">
          <h2 className="text-center text-5xl font-semibold mb-12">Recent Blogs</h2>
            ...
            {[...Array(cardCount)].map((_, idx) => (
              <div key={idx} className="bg-white shadow-lg rounded-lg p-4">
                <Link to={`/blog/${idx}`} className="text-black hover:text-red-500 cursor-pointer">
                  <h3 className="font-semibold text-lg text-center">Amplify your blockchain team </h3>
                </Link>
                <Link to={`/blog/${idx}`}>
                  <img
                    src={image}
                    alt="Blog post"
                    className="w-full rounded-lg mt-3 h-44 object-cover hover:opacity-75 transition duration-300"
                  />
                </Link>
                <div className="flex justify-center tracking-widest pt-2 space-x-3 text-sm text-gray-500 mt-2 items-center">
                ...
                <p className="w-full mx-auto text-gray-600 mt-3 text-center">
